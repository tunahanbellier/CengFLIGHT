...
class MultiGraph
{
    ...
    protected:
    public:
    std::vector<GraphVertex> GetVertexList2() const ;
    int GetVertexIndex(const std::string vertexname);
    std::vector<GraphVertex> GetVertexList();
    //void helperdepth(int& count  , int& index, const std::string& edgeName, std::vector<std::string>& visited) const;
